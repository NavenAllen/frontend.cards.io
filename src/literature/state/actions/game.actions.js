...
function playAskSuccess() {
    return dispatch => dispatch({ type: literatureGameConstants.CREATE_GAME_SUCCESS})
}

function playAskFailure(errorDetails) {
    return dispatch => dispatch({ type: literatureGameConstants.CREATE_GAME_FAILURE, errorDetails })
}